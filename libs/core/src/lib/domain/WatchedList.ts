export abstract class WatchedList<T> {
  ...
  private isDirtyItem(item: T): boolean {
    return this.dirtied
      .filter((v: T) => this.compareItems(item, v))
      .length !== 0;
  }
  ...
  private removeFromDirtied(item: T): void {
    this.dirtied = this.dirtied
      .filter((v) => !this.compareItems(v, item));
  }
  ...
  public remove(item: T): void {
    ...
    if (this.isDirtyItem(item)) {
      this.removeFromDirtied(item);
    }
  }

  public dirty(item: T): void {
    if (this.isCurrentItem(item) && !this.isNewItem(item)) {
      this.dirtied.push(item);
    }
  }
}
