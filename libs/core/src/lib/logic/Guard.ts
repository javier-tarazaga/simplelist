...
export class Guard {
  ...
  public static againstType(argument: any, type: string, argumentName: string): IGuardResult {
    if (typeof argument !== type) {
      return { succeeded: false, message: `${argumentName} wrong type, expecting: ${type}` };
    }
    return { succeeded: true };
  }
